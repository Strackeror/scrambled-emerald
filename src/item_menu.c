...
static EWRAM_DATA u32 sKeyItemWheelExtraPalette[8] = {0};
    ...
    case 0:
    {
        ...
        for (i = 0; i < MAX_REGISTERED_ITEMS; i++) {
            ...
            if (!gSaveBlock1Ptr->registeredItems[i] || !CheckBagHasItem(gSaveBlock1Ptr->registeredItems[i], 1))
                continue;
            tIconWindow[i] = j = AddWindowParameterized(0, sKeyItemBoxXPos[i] / 8 - 2, sKeyItemBoxYPos[i] / 8 - 2, 4, 4, i == 3 ? 13 : 13 + i, 16*(i+15));
            if (j == WINDOW_NONE)
                continue;
            PutWindowTilemap(j);

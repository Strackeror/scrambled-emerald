...
static u32 AI_CalcMoveEffectScore(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    for (i = 0; i < additionalEffectCount; i++)
    {
        ...
        bool8 moveTargetSelf = GetMoveTarget(move) == MOVE_TARGET_USER;
        ...
        if (gMovesInfo[move].additionalEffects[i].self || moveTargetSelf)
        {
            ...
            if (aiData->abilities[battlerAtk] != ABILITY_CONTRARY)
            {
                ...
                case MOVE_EFFECT_SP_DEF_PLUS_ARG:
                    StageStatId = STAT_CHANGE_ATK_2 + additionalEffect->moveEffect - MOVE_EFFECT_ATK_PLUS_ARG;
                    ADJUST_SCORE(IncreaseStatUpScore(battlerAtk, battlerDef, StageStatId));
                    break;
